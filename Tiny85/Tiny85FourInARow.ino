...
static const byte font[][8] PROGMEM = {
  ...
  {255,195,165,153,153,165,195,255},     //X
  {255,153,165,195,195,165,153,255},     //O
...
void setup() {
  ...
  boardinit(board);
  showboard(board);
  PCMSK = 0b00000111;   // pin change mask: listen to buttons
...
void playermove(){      //player has 'X'
  ...
  while(mv==0){
    int a;
    if(digitalRead(LEFTBUTTON)){      //is left pressed?
      delay(10);
      if(digitalRead(LEFTBUTTON)){    //debounce
        ppos=ppos-1;
    ...
    if(digitalRead(RIGHTBUTTON)){      //is right pressed?
      delay(10);
      if(digitalRead(RIGHTBUTTON)){    //debounce
        ppos=ppos+1;
    ...
    if(digitalRead(MIDDLEBUTTON)){      //is middle pressed?
      delay(10);
      if(digitalRead(MIDDLEBUTTON)){    //debounce
        if(!boardplay(board,ppos,'X')){
          ...
          showplayer(10,-1);
          while(digitalRead(MIDDLEBUTTON)){}      //wait till button released
          delay(100);
        }
      }
    }
